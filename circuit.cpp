...
int Sweep::sign(int x) { return x ? (x > 0 ? 1 : -1) : 0; }
...
void Sweep::cal_xor_and_pis(int x, int y, int &pis, double &score) {
  ...
  while (l <= r) {
    ...
    if (var_stamp[a] != stamp)
      q[++r] = a, var_stamp[a] = stamp;
    if (var_stamp[b] != stamp)
      q[++r] = b, var_stamp[b] = stamp;
  }
  for (int i = 1; i <= r; i++)
    par[q[i]] = q[i], xors_bot[q[i]] = 0;
  for (int i = 1; i <= r; i++) {
    int u = q[i];
    if (gate[cell[u].gate].type != Xor)
      continue;
    // need fixed ins = 2
...
bool Sweep::match_xor(int x) { // check clause x, -y, -z <==> xor, y = AND(a,
                               // ¬b), z = AND(¬a, b), x = AND(¬y, ¬z)
  int y = gate[x][0], z = gate[x][1];
  if (y >= 0 || z >= 0)
    return false; // Must be AND(¬y, ¬z)
  int gy = cell[-y].gate, gz = cell[-z].gate;
  if (!gy || !gz || gate[gy].type || gate[gy].type)
    return false; // Both must exist and be AND gates (type = 0)
  ...
  if (u1 == -v2 && v1 == -u2)
    goto doit;
  return false;
doit: // 把xor内部的两个gate给设成unuse了，如果这两个门没有别的fanouts。把输出直接和输入相连，类型设成xor
  if (cell[-y].fanouts <= 1)
    cell[-y].gate = 0, gate[gy].type = UNUSE;
  if (cell[-z].fanouts <= 1)
    cell[-z].gate = 0, gate[gz].type = UNUSE;
...
int find_par(int x) {
  if (par[x] != x)
    par[x] = find_par(par[x]); // Path compression
  return par[x];
}
