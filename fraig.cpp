...
bool Sweep::check_var_struct(int x, int y) {
  ...
  while (!q1.empty() && !q2.empty()) {
    ...
    if (gt1.ins != gt2.ins || gt1.type != gt2.type)
      return false;
    if (gt1.ins == 0)
      continue;
    int a = gt1[0], b = gt1[1];
    ...
    if (size[va] != size[vc])
      std::swap(c, d), std::swap(vc, vd);
    if (size[va] != size[vc] || size[vb] != size[vd])
      return false;
    if (size[va] == size[vb]) {
      if (sign(a) != sign(c))
        std::swap(c, d), std::swap(vc, vd);
    ...
    if (sign(a) != sign(c) || sign(b) != sign(d))
      return false;
    if (use1[va] != use2[vc] || use1[vb] != use2[vd])
      return false;
    if (!use1[va])
      q1.push(va), q2.push(vc), use1[va] = use2[vc] = true;
    if (!use1[vb])
      q1.push(vb), q2.push(vd), use1[vb] = use2[vd] = true;
  }
...
/* bool Sweep::seq_check_var_equal(int x, int y) {
    1. Build cone of influence (COI) for variables x and y
    2. Analyze structure to decide: EPCEC vs SAT
    3. If suitable for EPCEC: use EPCEC, Otherwise build SAT miter and solve
    4. Return whether x ≡ y
} */
...
bool Sweep::seq_check_var_equal(int x, int y) {
  ...
  while (l <= r) {
    ...
    if (gt.ins == 1) {
      int a = gt[0];
      // u ↔ a (equivalence)
      kissat_add(ksolver, u), kissat_add(ksolver, -a);
      kissat_add(ksolver, 0); // u ∨ ¬a
      kissat_add(ksolver, -u), kissat_add(ksolver, a);
      kissat_add(ksolver, 0); // ¬u ∨ a
      a = abs(a);
      if (!used[a])
    ...
    if (gt.type == And) {
      // u ↔ (a ∧ b)
      kissat_add(ksolver, -u);
      kissat_add(ksolver, a);
      kissat_add(ksolver, 0); // ¬u ∨ a
      kissat_add(ksolver, -u);
      kissat_add(ksolver, b);
      kissat_add(ksolver, 0); // ¬u ∨ b
      kissat_add(ksolver, -a);
      kissat_add(ksolver, -b);
      kissat_add(ksolver, u);
      kissat_add(ksolver, 0); // ¬a ∨ ¬b ∨ u
    } else {
      assert(gt.type == Xor);
      // u ↔ (a ⊕ b)
      kissat_add(ksolver, -a);
  ...
  // 先初始化，par的返回值是节点index的parrent节点（先赋值为自己），xors_bot是节点的xor
  // density，先赋值为0
  for (int i = 1; i <= r; i++)
    par[q[i]] = q[i], xors_bot[q[i]] = 0;
  for (int i = 1; i <= r; i++) {
    ...
    if (gate[c].type == Xor) {
      ...
      // 只有两个子节点都是xor时才继续向下找xor-chain
      if (!c1 || !c2 || gate[c1].type != Xor || gate[c2].type != Xor)
        continue;
      // 第一次遇到a的时候find_par(a)返回的是a本身，但后面就返回的是它的root
      // parrent，另一边也是（par[root]=root）这个赋值不改变系统state
      par[find_par(a)] = find_par(u);
      par[find_par(b)] = find_par(u);
    } else if (gate[c].type == And) {
      if (is_input[abs(gate[c][0])] && is_input[abs(gate[c][1])])
        ++connect;
    }
    // if (find_par(x) == find_par(y)) printf("c %d %d %d %d cs\n", i, u, a, b);
  }
  // 只会把所有的xor density都叠加在xorchain的root节点par[u]上
  for (int i = 1; i <= r; i++) {
    int u = q[i];
    if (find_par(u) == u)
      continue;
  ...
  for (int i = 1; i <= r; i++) {
    ...
    if (xors_bot[u] <= 2)
      continue;
    bucket[++xors_bot[u]]++; // Count components of each size
    mx = std::max(mx, xors_bot[u]);
    // printf("%d ", xors_bot[u]);
  }
  // puts("");
  // Logarithmic scaling for large components
  double score = mx;
  for (int i = 3; i < mx; i++) {
    bucket[i + 1] += bucket[i] >> 2; // Promote 1/4 of size-i to size-(i+1)
    bucket[i] %= 2;
  }
  while (bucket[mx] >= 4) // Each factor of 4 adds 1 to score
    score++, bucket[mx] /= 4;
  for (int i = 1; i <= mx; i++)
    bucket[i] = 0;   // cleaning
  score /= 1.0 * is; // Normalize by input count
  ...
  //     kissat_add(ksolver, -x); kissat_add(ksolver, y); kissat_add(ksolver,
  //     0); kissat_add(ksolver, x); kissat_add(ksolver, -y);
  //     kissat_add(ksolver, 0);
  // }
