...
// topological traversal with memory management and early termination
bool Sweep::_simulate(Bitset **result, void *_pool) {

  BitsetPool *pool = (BitsetPool *)_pool;
  // Reference count: how many gates still need variable i
  ...
  for (int i = 1; i <= gate.size(); i++)
    topo_counter[i] = 0; // How many inputs of gate c have been processed
  ...
  for (int i = 0; i < epcec_in.size(); i++) {
    ...
    q.push(epcec_in[i]);
    // Status: 0=unprocessed, 1=computed,
    // 2=input，输入因为可能需要生成反例，所以要被特殊对待
    used[epcec_in[i]] = 2;
  }
  ...
  while (!q.empty()) {
    int u = q.front();
    q.pop();
    // Early termination: stop when all outputs computed
    int flag = 1;
    ...
    if (flag)
      break;
    // Process all gates that use variable u as input
    for (int i = 0; i < inv_C[u].size(); i++) {
      int c = inv_C[u][i];
      // 如果相等了说明他两个输入的值都已经仿出来了
      if (++topo_counter[c] != gate[c].ins)
        continue;
      int v = abs(gate[c].out);
      if (!active[v])
        continue;
      // Skip variables outside COI,
      // 这个stamp是在外层的seq_check_var_equal里被修改（+1），然后所有在匹配对coi里的nodes都会被赋新的stamp值
      if (var_stamp[v] != stamp)
        continue;
      q.push(v), used[v] = 1;
      result[v] = pool->allocate();
      if (gate[c].ins == 1) { // 输入只有一个node，a=b or a=~b
        int l1 = gate[c][0], v1 = abs(l1);
        result[v]->eqs(*result[v1], sign(gate[c].out) * sign(l1));
        fanouts[v1]--;
        if (fanouts[v1] == 0 && used[v1] != 2)
          pool->release(result[v1]);
        continue;
      }
      // 输入有两个
      ...
      fanouts[v1]--;
      fanouts[v2]--;
      // 内存优化，当这个节点不再被需求，就提前释放掉
      if (fanouts[v1] == 0 && used[v1] != 2)
        pool->release(result[v1]);
      if (fanouts[v2] == 0 && used[v2] != 2)
        pool->release(result[v2]);
    }
  }

  bool res = true;
  // 如果是x==y的output就检查x和y所有的word是不是相等，
  // 如果是单个output，就检查所有比特位是不是0，不是0的话就提取sat model
  if (epcec_out.size() == 2) {
...
bool Sweep::pcec() {
  ...
  w = 1ll << R; // 所有随机组合，如果R=10，就是2^10=1024个
  // 1024个比特，用16个ull
  // word来存，注意这里一个1024的赋值组合就是一个16元素的array存在bitset类型的数据里，
  // 而global
  // pool里先开辟了默认200个这样的空向量（它假设的aiger的大小，我觉得有点小了）等待被赋值（后面的random()）
  // 这样做目的是可以更快的处理数据
  global_pool.init(w);
  Bitset **result = new Bitset *[maxvar + 1];
  ...
  A.allocate(w);
  A.set();
  A.hash(); // set是真正的赋值命令，这里赋值的是一个1024个1的bitset向量
  B.allocate(w);
  B.reset();
  B.hash(); // 同上，只不过是0000向量
  // hash的目的是比较的两个bitsets时候方便些，不用比较16个word的每一个，看后面代码其实最终还是要check实际赋值的，hash值只是一个filter
  ...
  for (int i = 1; i <= maxvar; i++) {
    ...
    if (!c || gate[c].type == UNUSE)
      continue;
    classes[0].push_back(i); // 初始化的时候所有gate都是放在一个class里的
  }
    ...
    while (!q.empty()) {
      ...
      for (int i = 0; i < inv_C[u].size(); i++) {
        ...
        if (topo_counter[c] == gate[c].ins) {
          ...
          fanouts[v1]--;
          fanouts[v2]--;
          if (fanouts[v1] == 0 && !is_input[v1])
            global_pool.release(result[v1]);
          if (fanouts[v2] == 0 && !is_input[v2])
            global_pool.release(result[v2]);
...
//         int x = equal_pairs[i].second.first, y =
//         equal_pairs[i].second.second; if (!active[x] || !active[y]) {
//             task_sign[i] = 0;
//             printf("c [%d/%d] trying to prove equivalance of pair (%d, %d)
//             (size: %d %d)\n", i+1, equal_pairs.size(), x, y, size[x],
//             size[y]); printf("c can skip cs not active
//             +++++++++++++++++++++++++++\n"); skip_act++; continue;
//         }
...
//         printf("c [%d/%d] trying to prove equivalance of pair (%d, %d) (size:
...
/* bool Sweep::seq_CEC() {
    1. Initialize pair checking tasks
    2. For each candidate equivalent pair (x,y):
       a. Skip if already processed or inactive
       b. Try fast structural checks first
       c. Use expensive SAT/EPCEC if needed
       d. If equivalent, merge variables
    3. Build and solve final miter problem
...
bool Sweep::seq_CEC() {
  ...
  // %d) (size: %d %d)\n", 992, 993, size[992], size[993]); int resc =
  // seq_check_var_equal(992, 993); printf("res is %d, use time: %d\n\n", resc,
  // std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()
  // - clk_st).count());
  ...
  verified:
    double use_time = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  // printf("c pairs (non-active, struct-equal, same-fanin, epcec, sat-solver)
  // %d %d %d %d %d %d\n", info.pairs, info.pairs_act, info.pairs_stru,
  // info.pairs_fanin, info.pairs_ec, info.pairs_sat); printf("c use time
  // (epcec, sat-solver) %.2lf %.2lf\n", info.times_ec, info.times_sat);
  // printf("c verified pairs (epcec, sat-solver) %d %d %d\n", info.pairs_ver,
  // info.pairs_ec_ver, info.pairs_sat_ver);
